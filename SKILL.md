...
description: >-
  ...
  this error already", "remember how we fixed Y". Recalls relevant notes at the
  start of a non-trivial task and captures a new note at the end of one worth
  remembering. Governs what is remembered across sessions, not how code is
  written — it never competes with the kanso skills for code-writing triggers.
---
...
Skip — over-capture is the failure mode that turns the library to sludge:
...
- Markdown only — notes and index. No HTML store, no JSON, no database, no
